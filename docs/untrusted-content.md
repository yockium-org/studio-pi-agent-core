...
Public redaction/signal helpers accept unknown content values and safely stringify non-string input before scanning, matching envelope behavior. Envelope creation also tolerates missing, primitive, or non-options-shaped runtime values by treating the value itself as untrusted content. Rendering/result helpers accept unknown runtime envelope values, normalize manually supplied envelope content and signal diagnostics, and fall back to `unknown` / `Untrusted content` for missing envelope objects, so JS callers are not required to construct perfect TypeScript-shaped envelopes. Content and metadata are normalized into JSON-friendly values, including Date, URL, RegExp, Set, and Map values; metadata is snapshotted and frozen, and non-object metadata is wrapped under a `value` field. Signal diagnostics remain redacted even if a caller disables body redaction with `redactSensitiveContent: false`.
